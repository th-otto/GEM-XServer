...
#define ATOM_TABLE \
	...
	ENTRY (XA_COLORMAP) \
	ENTRY (XA_CURSOR) \
	ENTRY (XA_CUT_BUFFER0) \
	ENTRY (XA_CUT_BUFFER1) \
	ENTRY (XA_CUT_BUFFER2) \
	ENTRY (XA_CUT_BUFFER3) \
	ENTRY (XA_CUT_BUFFER4) \
	ENTRY (XA_CUT_BUFFER5) \
	ENTRY (XA_CUT_BUFFER6) \
	ENTRY (XA_CUT_BUFFER7) \
	ENTRY (XA_DRAWABLE) \
	ENTRY (XA_FONT) \
	ENTRY (XA_INTEGER) \
	ENTRY (XA_PIXMAP) \
	ENTRY (XA_POINT) \
	ENTRY (XA_RECTANGLE) \
	ENTRY (XA_RESOURCE_MANAGER) \
	ENTRY (XA_RGB_COLOR_MAP) \
	ENTRY (XA_RGB_BEST_MAP) \
	ENTRY (XA_RGB_BLUE_MAP) \
	ENTRY (XA_RGB_DEFAULT_MAP) \
	ENTRY (XA_RGB_GRAY_MAP) \
	ENTRY (XA_RGB_GREEN_MAP) \
	ENTRY (XA_RGB_RED_MAP) \
	ENTRY (XA_STRING) \
	ENTRY (XA_VISUALID) \
	ENTRY (XA_WINDOW) \
	ENTRY (XA_WM_COMMAND) \
	ENTRY (XA_WM_HINTS) \
	ENTRY (XA_WM_CLIENT_MACHINE) \
	ENTRY (XA_WM_ICON_NAME) \
	ENTRY (XA_WM_ICON_SIZE) \
	ENTRY (XA_WM_NAME) \
	ENTRY (XA_WM_NORMAL_HINTS) \
	ENTRY (XA_WM_SIZE_HINTS) \
	ENTRY (XA_WM_ZOOM_HINTS) \
	ENTRY (XA_MIN_SPACE) \
	ENTRY (XA_NORM_SPACE) \
	ENTRY (XA_MAX_SPACE) \
	ENTRY (XA_END_SPACE) \
	ENTRY (XA_SUPERSCRIPT_X) \
	ENTRY (XA_SUPERSCRIPT_Y) \
	ENTRY (XA_SUBSCRIPT_X) \
	ENTRY (XA_SUBSCRIPT_Y) \
	ENTRY (XA_UNDERLINE_POSITION) \
	ENTRY (XA_UNDERLINE_THICKNESS) \
	ENTRY (XA_STRIKEOUT_ASCENT) \
	ENTRY (XA_STRIKEOUT_DESCENT) \
	ENTRY (XA_ITALIC_ANGLE) \
	ENTRY (XA_X_HEIGHT) \
	ENTRY (XA_QUAD_WIDTH) \
	ENTRY (XA_WEIGHT) \
	ENTRY (XA_POINT_SIZE) \
	...
	ENTRY (XA_NOTICE) \
	ENTRY (XA_FONT_NAME) \
	ENTRY (XA_FAMILY_NAME) \
	ENTRY (XA_FULL_NAME) \
	ENTRY (XA_CAP_HEIGHT) \
	ENTRY (XA_WM_CLASS) \
	ENTRY (XA_WM_TRANSIENT_FOR)

#define ENTRY(e) static struct { ATOM_BASE; char Name[sizeof(#e)]; } xa_##e = { 0, NULL, NULL, 0, 0, #e };
ATOM_TABLE
/* --- Extra Atom Definitions */
static struct { ATOM_BASE; char Name[sizeof("WM_PROTOCOLS")]; } xa_WM_PROTOCOLS =  { 0, NULL, NULL, 0, 0, "WM_PROTOCOLS" };
static struct { ATOM_BASE; char Name[sizeof("WM_DELETE_WINDOW")]; } xa_WM_DELETE_WINDOW =  { 0, NULL, NULL, 0, 0, "WM_DELETE_WINDOW" };
#undef ENTRY

ATOM *ATOM_Table[MAX_ATOM + 2] = {
# define ENTRY(e) (ATOM *) &xa_##e,
	ATOM_TABLE
...
#	endif
	...
	memset(ATOM_Table + LAST_PREDEF_ATOM + 1, 0, (MAX_ATOM - LAST_PREDEF_ATOM) * sizeof(ATOM *));
...
Atom AtomGet(const char *name, size_t len, BOOL onlyIfExists)
{
	...
	while (end >= beg)
	{
		...
		if (!dir)
			dir = len - _ATOM_Sort[num]->Length;
		if (dir > 0)
			beg = num + 1;
		else if (dir < 0)
			end = num - 1;
		else
			return _ATOM_Sort[num]->Id;
	}
	if (dir > 0)
		num++;

	if (onlyIfExists || ATOM_Count >= MAX_ATOM || (atom = malloc(offsetof(ATOM, Name) + len + 1)) == NULL)
		return None;
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_InternAtom(CLIENT *clnt, xInternAtomReq *q)
{
	/*
	...
	if (!q->nbytes)
	{
		Bad(BadValue, None, X_InternAtom, "_():\n          zero length string.");
...
#	endif
		...
		{
			Bad(BadAlloc, None, X_InternAtom, "_('%.*s'):\n"
				"          %s.", q->nbytes, name,
				(ATOM_Count >= MAX_ATOM ? "maximum number reached" : "memory exhausted"));
		} else
		{
			ClntReplyPtr(InternAtom, r, 0);

			DEBUG(X_InternAtom, "('%.*s') -> %lu%s", q->nbytes, name, atom, cnt < ATOM_Count ? "+" : "");
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_GetAtomName(CLIENT *clnt, xGetAtomNameReq *q)
{
	/*
	...
	} else
	{
		...
		DEBUG(X_GetAtomName, "(%lu) ->'%.*s'", q->id, (int) len, ATOM_Table[q->id]->Name);

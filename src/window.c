...
static short _MAP_Inc;
static short _MAP_IncX;
static short _MAP_IncY;
static short _WIND_RootX2;
static short _WIND_RootY2;
	...
	} else
	{
		if (WIND_Root.StackBot)
		{
			...
			while (w)
			{
				...
				if (w->StackBot)
					sprintf(w_f, "W:%X", w->StackBot->Id);
				if (w->StackTop)
					sprintf(w_l, "W:%X", w->StackTop->Id);
...
#if 0
			x_printf("  remove Propert%s:", (WIND_Root.Properties->Next ? "ies" : "y"));
			while (WIND_Root.Properties)
			{
				x_printf(" '%s'(%s)", ATOM_Table[WIND_Root.Properties->Name]->Name, ATOM_Table[WIND_Root.Properties->Type]->Name);
...
BOOL WindButton(CARD16 prev_mask, int count)
{
	...
	CARD8 butt_r = (prev_mask & Button1Mask ? Button1 :
					prev_mask & Button2Mask ? Button2 : prev_mask & Button3Mask ? Button3 : None);
	CARD8 butt_p = (MAIN_KeyButMask & Button1Mask ? Button1 :
					MAIN_KeyButMask & Button2Mask ? Button2 : MAIN_KeyButMask & Button3Mask ? Button3 : None);
	...
	if ((MAIN_KeyButMask & K_ALT) && (MAIN_KeyButMask & Button2Mask) && wind)
	{
		...
		x_printf("\nW:%X 0x%lX #%i [%i,%i/%i,%i/%i] * %i \n",
			...
			wind->Rect.g_x, wind->Rect.g_y, wind->Rect.g_w, wind->Rect.g_h, wind->BorderWidth, wind->Depth);
		if (wind->hasBorder || wind->hasBackGnd)
		{
			...
			if (wind->hasBackPix)
			{
				x_printf("backgnd: P:%X [%i,%i] * %i", wind->Back.Pixmap->Id,
					wind->Back.Pixmap->W, wind->Back.Pixmap->H, wind->Back.Pixmap->Depth);
		...
		if (wind->Parent)
		{
			...
			if (wind->PrevSibl)
				x_printf("   prev: W:%X", wind->PrevSibl->Id);
			if (wind->NextSibl)
				x_printf("   next: W:%X", wind->NextSibl->Id);
			x_printf("\n");
		}
	...
	if (!_WIND_PgrabWindow && wind)
	{
		...
		if (butt_r)
		{
			EvntPropagate(wind, ButtonReleaseMask, ButtonRelease, wind->Id, r_xy, WindPointerPos(wind), butt_r);
			if (!butt_p)
			{
				return xFalse;
			}
...
#define MASK (ButtonPressMask|ButtonReleaseMask)
			if (!_Wind_PgrabMatch(wind, butt_p, MAIN_Key_Mask)
				&& (wind->u.List.AllMasks & MASK) == MASK && (clnt = EvntClient(wind, MASK)))
			{
				_Wind_PgrabSet(clnt, wind, NULL, wind->u.List.AllMasks, xTrue, MAIN_TimeStamp, xTrue);
			}
#undef MASK
		}
	}
			...
			if (butt_r)
			{
				...
				if ((!_WIND_PgrabOwnrEv ||
					 !(wnd_r = EvntSearch(wind, _WIND_PgrabClient,
										  ButtonReleaseMask))) && (_WIND_PgrabEvents & ButtonReleaseMask))
				{
				...
				if (wnd_r)
				{
					...
					EvntKeyButMotion(_WIND_PgrabClient, ButtonRelease, wnd_r->Id, WindChildOf(wnd_r, wind), r_xy, w_xy, butt_r);
				}
		...
		if (w_id && butt_p)
		{
			...
			if ((!_WIND_PgrabOwnrEv ||
				 !(wnd_p = EvntSearch(wind, _WIND_PgrabClient,
									  ButtonPressMask))) && (_WIND_PgrabEvents & ButtonPressMask))
			{
			...
			if (wnd_p)
			{
				...
				EvntKeyButMotion(_WIND_PgrabClient, ButtonPress, wnd_p->Id, c_id, r_xy, w_xy, butt_p);
			}
			if (count > 1)
			{
				WINDOW *wnd_r = NULL;
				...
				if (wnd_r == wnd_p)
				{
					EvntKeyButMotion(_WIND_PgrabClient, ButtonRelease, wnd_r->Id, c_id, r_xy, w_xy, butt_p);
				} else if (wnd_r)
				{
					PXY e_xy = WindPointerPos(wnd_r);

					EvntKeyButMotion(_WIND_PgrabClient, ButtonRelease, wnd_r->Id, WindChildOf(wnd_r, wind), r_xy, e_xy, butt_p);
				}
				if (wnd_p)
				{
					EvntKeyButMotion(_WIND_PgrabClient, ButtonPress, wnd_p->Id, c_id, r_xy, w_xy, butt_p);
	...
	if (butt_p)
	{
		...
		EvntPropagate(wind, ButtonPressMask, ButtonPress, wind->Id, r_xy, w_xy, butt_p);
		if (count > 1)
		{
			EvntPropagate(wind, ButtonReleaseMask, ButtonRelease, wind->Id, r_xy, w_xy, butt_r);
			EvntPropagate(wind, ButtonPressMask, ButtonPress, wind->Id, r_xy, w_xy, butt_p);
		}
	}
...
/* ------------------------------------------------------------------------------ */
static void _Wind_setup(CLIENT *clnt, WINDOW *w, CARD32 mask, CARD32 *val, CARD8 req)
{
	if (clnt->DoSwap)
	{
		CARD32 m = mask;
	...
	if (mask & CWBackPixmap)
	{
		if (*val != None && *val != ParentRelative)
		{
			...
			} else if (pmap->Depth != w->Depth)
			{
				...
				PRINT(0, " ");
#endif
				Bad(BadMatch, None, req, "_          background depth %u not %u.", pmap->Depth, w->Depth);
				return;
			} else
			{
#if 0
...
/* ============================================================================== */
BOOL WindSetMapped(WINDOW *wind, BOOL visible)
{
	BOOL redraw = xFalse;
...
/* ============================================================================== */
void WindClrMapped(WINDOW *wind, BOOL by_conf)
{
...
/* ------------------------------------------------------------------------------ */
static void _Wind_Unmap(WINDOW *wind, BOOL visible, BOOL ptr_check)
{
...
/* ============================================================================== */
void WindDelete(WINDOW *wind, CLIENT *clnt)
{
	BOOL enter = xFalse;
	WINDOW *pwnd = wind->Parent;
	WINDOW *next = NULL;
	...
	while (1)
	{
		...
		{
			x_printf("\033pPANIC\033q wind_destroy()" " W:%X must be excluded from deleting!\n", wind->Id);
			exit(1);
		}
		if (wind->StackBot || wind->StackTop)
		{
			x_printf("\033pPANIC\033q wind_destroy()"
				...
				wind->Id, (wind->StackBot ? wind->StackBot->Id : 0), (wind->StackTop ? wind->StackTop->Id : 0));
			exit(1);
		}
		...
		while (wind->Properties)
			PropDelete(&wind->Properties);
		if (wind->nSelections)
			SlctClear(wind);
		if (wind->Cursor)
		...
		if (wind->PrevSibl)
			...
			pwnd->StackBot = wind->NextSibl;
		if (wind->NextSibl)
			wind->NextSibl->PrevSibl = wind->PrevSibl;
		else
			pwnd->StackTop = wind->PrevSibl;
...
/* ============================================================================== */
BOOL WindCirculate(WINDOW *wind, CARD8 place)
{
	BOOL notify = xFalse;
	BOOL redraw = xFalse;
	...
	{									/* (place == PlaceOnBottom) */
		...
		if (wind->Handle > 0)
			wind_set(wind->Handle, WF_BOTTOM, 0, 0, 0, 0);
		else
			redraw = notify;
	}
...
/* ------------------------------------------------------------------------------ */
static void _Wind_Resize(WINDOW *wind, GRECT *diff)
{
	...
	if ((wind = wind->StackBot))
		do
		{
			...
			if (wind->WinGravity == NorthGravity ||
				wind->WinGravity == CenterGravity || wind->WinGravity == SouthGravity)
			{
			...
			if (wind->WinGravity == WestGravity || wind->WinGravity == CenterGravity || wind->WinGravity == EastGravity)
			{
			...
			if (notify)
			{
				PXY pos = { wind->Rect.g_x - wind->BorderWidth, wind->Rect.g_y - wind->BorderWidth };
...
/* ============================================================================== */
void WindResize(WINDOW *wind, GRECT *diff)
{
	GRECT curr;
...
/* ------------------------------------------------------------------------------ */
void RQ_CreateWindow(CLIENT *clnt, xCreateWindowReq *q)
{
	WINDOW *pwnd = NULL;
	WINDOW *wind = NULL;
	...
	} else if ((pwnd = WindFind(q->parent)) == NULL)
	{
		Bad(BadWindow, q->parent, X_CreateWindow, "_");
	} else if ((short) q->width <= 0 || (short) q->height <= 0)
	{
		Bad(BadValue, (short) ((short) q->width <= 0 ? q->width : q->height), X_CreateWindow, "_ width = %i height = %i", (short) q->width, (short) q->height);

		/* check visual */
		/* check depth */
	} else if ((wind = XrscCreate(WINDOW, q->wid, clnt->Drawables, 0)) == NULL)
	{
		Bad(BadAlloc, None, X_CreateWindow, "_(W:%lX)", q->wid);
	} else
	{
#if 0
		PRINT(CreateWindow, "-(%u) W:%lX [%i,%i/%u,%u/%u:%u] on W:%lX with V:%lX",
			  q->class, q->wid, q->x, q->y, q->width, q->height, q->borderWidth, q->depth, q->parent, q->visual);
#endif
		wind->isWind = xTrue;

		wind->ClassInOut = (q->class == CopyFromParent ? pwnd->ClassInOut : q->class == InputOutput ? xTrue : xFalse);
		...
		wind->Depth = (!q->depth && wind->ClassInOut ? pwnd->Depth : q->depth);
		...
		{
			Bad(BadAlloc, None, X_CreateWindow, "_(W:%lX): AES", q->wid);
		...
		{
			EvntCreateNotify(pwnd, wind->Id, &wind->Rect, wind->BorderWidth, wind->Override);
		}
	}
}

/* ------------------------------------------------------------------------------ */
void RQ_MapWindow(CLIENT *clnt, xMapWindowReq *q)
{
...
#	endif
	...
	{
		DEBUG(X_MapWindow, " W:%lX (%i)", q->id, wind->Handle);
...
/* ------------------------------------------------------------------------------ */
void RQ_MapSubwindows(CLIENT *clnt, xMapSubwindowsReq *q)
{
	WINDOW *wind = WindFind(q->id);
...
#	endif
	} else if (wind->StackTop)
	{
		...
		DEBUG(X_MapSubwindows, " W:%lX", q->id);
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_UnmapWindow(CLIENT *clnt, xUnmapWindowReq *q)
{
	/*
...
#	endif
	...
	{
		DEBUG(X_UnmapWindow, " W:%lX #%i", q->id, wind->Handle);
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_UnmapSubwindows(CLIENT *clnt, xUnmapSubwindowsReq *q)
{
	/*
		...
		do
		{
			if (w->isMapped)
			{
				if (vis && (draw = !root))
				{
					...
					if (rect.g_x > work.g_x)
						rect.g_x = work.g_x;
					if (rect.g_w < (work.g_w += work.g_x))
						rect.g_w = work.g_w;
					if (rect.g_y > work.g_y)
						rect.g_y = work.g_y;
					if (rect.g_h < (work.g_h += work.g_y))
						rect.g_h = work.g_h;
				}
...
/* ------------------------------------------------------------------------------ */
void RQ_ConfigureWindow(CLIENT *clnt, xConfigureWindowReq *q)
{
	...
	} else
	{
		...
		if (q->mask & CWX)
		{
			d.g_x = (short) *(val++) + db - r->g_x;
		}
		if (q->mask & CWY)
		{
			d.g_y = (short) *(val++) + db - r->g_y;
		}
		if (q->mask & CWWidth)
		{
			d.g_w = (short) *(val++) - r->g_w;
		}
		if (q->mask & CWHeight)
		{
			d.g_h = (short) *(val++) - r->g_h;
		}
...
/* ------------------------------------------------------------------------------ */
void RQ_ChangeWindowAttributes(CLIENT *clnt, xChangeWindowAttributesReq *q)
{
	WINDOW *wind = WindFind(q->window);
	...
	} else
	{
		...
		_Wind_setup(clnt, wind, q->valueMask, (CARD32 *) (q + 1), X_ChangeWindowAttributes);

		if ((q->valueMask & CWOverrideRedirect) && WMGR_Active && !wind->isMapped && wind->Handle > 0)
		{
		...
		if (q->valueMask & CWCursor)
		{
			if (_WIND_PgrabWindow)
			{
				if (!_WIND_PgrabCursor && _Wind_IsInferior(wind, _WIND_PgrabWindow))
				{
		...
		if ((q->valueMask & (CWBackPixmap | CWBackPixel)) && wind->Id == ROOT_WINDOW)
		{
			WmgrSetDesktop(wind->hasBackGnd);
			if (desktop)
				PmapFree(desktop, NULL);
		}
...
/* ------------------------------------------------------------------------------ */
void RQ_CirculateWindow(CLIENT *clnt, xCirculateWindowReq *q)
{
	PRINT(-X_CirculateWindow, " W:%lX %s", q->window, (q->direction ? "RaiseLowest" : "LowerHighest"));
}

/* ------------------------------------------------------------------------------ */
void RQ_GetWindowAttributes(CLIENT *clnt, xGetWindowAttributesReq *q)
{
	WINDOW *wind = NULL;
	...
	} else
	{
		...
		DEBUG(X_GetWindowAttributes, " W:%lX", q->id);
		...
		if (wind)
		{
			...
			r->mapState = (!wind->isMapped ? IsUnmapped : WindVisible(wind) ? IsUnviewable : IsViewable);
...
/* ------------------------------------------------------------------------------ */
void RQ_DestroyWindow(CLIENT *clnt, xDestroyWindowReq *q)
{
	WINDOW *wind = WindFind(q->id);
	...
	} else
	{
		...
		DEBUG(X_DestroyWindow, " W:%lX #%i", q->id, wind->Handle);

		WindDelete(wind, NULL);

		if (pwnd)
			WindDrawSection(pwnd, &curr);
	}
}

/* ------------------------------------------------------------------------------ */
void RQ_DestroySubwindows(CLIENT *clnt, xDestroySubwindowsReq *q)
{
	PRINT(-X_DestroySubwindows, " W:%lX", q->id);
}

/* ------------------------------------------------------------------------------ */
void RQ_ReparentWindow(CLIENT *clnt, xReparentWindowReq *q)
{
	/*
	 * Window window:
	 * Window parent:
...
#	endif
	} else
	{
		...
		do
		{
			if (w == wind)
			{
				Bad(BadMatch, None, X_ReparentWindow, "_(): parent is inferior.");
				return;
			}
		} while ((w = w->Parent));
		/*........................................................................ */
		...
		if (wind->u.List.AllMasks & StructureNotifyMask)
		{
			...
			p.p_x = wind->Rect.g_x;
			p.p_y = wind->Rect.g_y;
			EvntReparentNotify(wind, StructureNotifyMask, wind->Id, pwnd->Id, p, wind->Override);
		}
		if (pwnd->u.List.AllMasks & SubstructureNotifyMask)
		{
			PXY p;
			...
			p.p_x = wind->Rect.g_x;
			p.p_y = wind->Rect.g_y;
			EvntReparentNotify(pwnd, SubstructureNotifyMask, wind->Id, pwnd->Id, p, wind->Override);
		}
...
/* ------------------------------------------------------------------------------ */
void RQ_QueryTree(CLIENT *clnt, xQueryTreeReq *q)
{
	WINDOW *wind = WindFind(q->id);
	...
	} else
	{
		...
		size_t size = 0;
		size_t bspc = clnt->oBuf.Size - (clnt->oBuf.Done + clnt->oBuf.Left) - sz_xQueryTreeReply;
		...
		while (chld)
		{
			...
			if (need > bspc)
			{
				r = ClntOutBuffer(&clnt->oBuf, sz_xQueryTreeReply + need, sz_xQueryTreeReply + size, xTrue);
				c_id = (CARD32 *) (r + 1) + r->nChildren;
				bspc = clnt->oBuf.Size - (clnt->oBuf.Done + clnt->oBuf.Left) - sz_xQueryTreeReply;
			}
...
/* ------------------------------------------------------------------------------ */
void RQ_ChangeSaveSet(CLIENT *clnt, xChangeSaveSetReq *q)
{

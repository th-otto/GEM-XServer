...
/* ------------------------------------------------------------------------------ */
static void _save_face(FILE *file, FONTFACE *face)
{
	...
	fprintf(file, "   %i, %i,%i, %i   %i,%i   %i,%i"
			...
			face->MaxWidth, face->MaxAsc, face->MaxDesc, face->MaxLftBr, face->MaxRgtBr);
}

/* ------------------------------------------------------------------------------ */
FONTFACE *_Font_Create(const char *name, size_t len, unsigned type, BOOL sym, BOOL mono)
{
...
void FontInit(short count)
{
	...
	if ((access(PATH_LibDir, R_OK | W_OK | X_OK) != 0 && mkdir(PATH_LibDir, S_IRWXU | S_IRWXG | S_IRWXO) != 0) ||
		(access(PATH_FontsDb, F_OK) == 0 && (access(PATH_FontsDb, W_OK) != 0 || (f_db = fopen(PATH_FontsDb, "r")) == NULL)))
	{
		x_printf("  \033pERROR\033q: Can't access %s.\n", PATH_LibDir);
		return;
	} else
	{
		int major = -1;
	...
	if (f_db)
	{
		...
		while (fgets(buf, sizeof(buf), f_db))
		{
			...
			if (sscanf(buf, "%i: %u,%u,%u%c%n", &i, &type, &isSymbol, &isMono, &c, &j) == 5 && c == ' ')
			{
				struct FONT_DB *db;
			...
			} else if (face == NULL)
			{
				...
				break;
							  ...
							  &face->MinWidth, &face->MinAsc, &face->MinDesc,
							  &face->MinLftBr, &face->MinRgtBr,
							  &face->MaxWidth, &face->MaxAsc, &face->MaxDesc, &face->MaxLftBr, &face->MaxRgtBr) == 18)
			{
				face->CharSet = NULL;
	...
	for (i = 1; i <= count; i++)
	{
		...
		const char *wght = info.style_name;
		const char *creg = "ISO8859";
		const char *cenc = "1";
		const char *fndr = NULL;
		const char *setw = NULL;
		const char *astl = NULL;
		char slnt[3] = "\0\0";
		char spcg[2] = "\0";
		unsigned int resx = 72;
		unsigned int resy = 72;
		BOOL isMono;
		BOOL isSymbol;
		...
		if (info.pt_cnt > 1 && info.pt_sizes[info.pt_cnt - 1] == info.pt_sizes[info.pt_cnt - 2])
		{
			info.pt_cnt--;
		}
		...
		if (!isSymbol)
		{
			if (info.format == 1 && info.font_name[0] == '›' && info.font_name[1] == '-')
			{
				latn = xTrue;
				cset = _FONT_ExtLatin;
			} else
		...
		if (f_db)
			fprintf(f_db, "%i: %u,%u,%u %s\n", info.id, info.format, isSymbol, isMono, info.file_name1);
...
#pragma GCC diagnostic warning "-Wformat-overflow"
		...
		for (j = 0; j < info.pt_cnt; ++j)
		{
			...
			if ((*list = _Font_Create(fhdr->fh_fmver, len, info.format, isSymbol, isMono)) != NULL)
			{
				...
				if (face->Type == 4)
				{
					/* For determining TrueType, finish the startup intro and */

...
/* ============================================================================== */
void FontDelete(p_FONT font, p_CLIENT clnt)
{
	XrscDelete(clnt->Fontables, font);
}

/* ------------------------------------------------------------------------------ */
static inline p_FONT FontFind(CARD32 id)
{
	p_FONT font = FablFind(id).Font;

	if ((DBG_XRSC_TypeError = (font && !font->isFont)))
		font = NULL;
	return font;
}
...
/* ============================================================================== */
BOOL FontValues(p_FONTABLE fabl, CARD32 id)
{
	p_FONTABLE font;
	BOOL ok = xTrue;
...
/* ============================================================================== */
/* FIXME: arr & str sometimes point to same memory */
...
/* ------------------------------------------------------------------------------ */
static void _Font_Alias(char *buf, const char *src, size_t len)
{
...
/* ------------------------------------------------------------------------------ */
static FONTFACE **_Font_Match(const char *pattern, FONTFACE *start)
{
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_ListFonts(CLIENT *clnt, xListFontsReq *q)
{
	/*
	...
	CARD16 num = 0;
	size_t size = 0;
	size_t have = clnt->oBuf.Size - (clnt->oBuf.Done + clnt->oBuf.Left) - sz_xListFontsReply;
	char buf[500] = "";

	PRINT(X_ListFonts, " '%.*s' max=%u", q->nbytes, (char *) (q + 1), q->maxNames);
	...
	while (face && num < q->maxNames)
	{
		if (!fnmatch(buf, face->Name, FNM_NOESCAPE | FNM_CASEFOLD))
		{
			...
			if (need > have)
			{
				...
				r = rr;
				list = (char *) (r + 1);
				have = clnt->oBuf.Size - (clnt->oBuf.Done + clnt->oBuf.Left) - sz_xListFontsReply;
			}
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_OpenFont(CLIENT *clnt, xOpenFontReq *q)
{
	/*
	...
	} else
	{
		...
		if (!face && sscanf(patt, "%ux%u", &w, &h) == 2)
		{
			...
			if (!face)
			{
				...
				while (*(prot = _Font_Match(buf, *prot)) && ((*prot)->Type < 2 || (*prot)->isSymbol))
					;
				if (!*prot)
				{
					prot = &_FONT_List;
					while (*prot && ((*prot)->Type < 2 || !(*prot)->isMono || (*prot)->isSymbol))
					{
						prot = &(*prot)->Next;
					}
				}
				if (*prot)
				{
					...
					if (!face)
					{
						Bad(BadAlloc, None, X_OpenFont, "_ (generic)");
						return;
					}
		...
		if (!face)
		{
			Bad(BadName, None, X_OpenFont, "_('%.*s')\n          -> '%s'", (int) q->nbytes, patt, buf);
		} else if (!(font = XrscCreate(FONT, q->fid, clnt->Fontables, 0)))
		{
			Bad(BadAlloc, None, X_OpenFont, "_");
		} else
		{
			font->isFont = xTrue;
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_CloseFont(CLIENT *clnt, xCloseFontReq *q)
{
	/*
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_QueryFont(CLIENT *clnt, xQueryFontReq *q)
{
	/*
	...
	} else
	{
		...
		while (1)
		{
			...
			if (!(*(atom++) = AtomGet("FONT", 4, xFalse)) || !(*(atom++) = AtomGet(face->Name, face->Length, xFalse)))
				break;
			r->nFontProps++;
			size += sizeof(Atom) * 2;
			break;
		...
		} else
		{
			...
			for (c = face->MinChr; c <= face->MaxChr; ++c)
			{
				vqt_width(GRPH_Vdi, (face->CharSet ? face->CharSet[c] : c), &w, &ld, &rd);
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_QueryTextExtents(CLIENT *clnt, xQueryTextExtentsReq *q)
{
	...
	} else
	{
		...
		size_t size = ((q->length * 4) - sizeof(xQueryTextExtentsReq)) - (q->oddLength ? 1 : 0);
		short str[size];
		short ext[8];
...
/* ------------------------------------------------------------------------------ */
void RQ_ListFontsWithInfo(CLIENT *clnt, xListFontsWithInfoReq *q)
{
	PRINT(-X_ListFontsWithInfo, " '%.*s' max=%u", q->nbytes, (char *) (q + 1), q->maxNames);
}


/* ------------------------------------------------------------------------------ */
void RQ_SetFontPath(CLIENT *clnt, xSetFontPathReq *q)
{
	PRINT(-X_SetFontPath, " (%u)", q->nFonts);
}

/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_GetFontPath(CLIENT *clnt, xGetFontPathReq *q)
{
	/*
	 ...
	 *
	 * Reply:
	 * CARD16 nPaths: number of directorys
	 */
	static char const src[] = "/c/gemsys";

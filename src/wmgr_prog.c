...
/* ------------------------------------------------------------------------------ */
static char *tos2unix(char *path)
{
	char *unx = path;
	...
	if (path[1] == ':')
	{
		...
		if (len >= 0 && (unx = malloc(len + strlen(path += 2))) != NULL)
		{
			char *u;
			char *p;

			if (len)
				u = (char *) memcpy(unx, buf, len) + len;
			else
				u = unx;
			*(u++) = '/';
...
/* ============================================================================== */
short WmgrLaunch(const char *prg, int argc, const char *arg_v[])
{
	char **src = environ;
	short pid = -1;
	...
	for (i = 0; src[i]; i++)
	{
		if (strncmp(src[i], "ARGV=", 5) == 0)
			break;
		len += strlen(src[i]) + 1;
	}
	for (i = 0; i < argc; i++)
	{
		if ((argv[i] = tos2unix((char *) arg_v[i])) == NULL)
		{
			argc = i;
			break;
		}

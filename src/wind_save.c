...
void WindSaveUnder(CARD32 id, GRECT *rect, short hdl)
{
	...
	if (addr)
	{
		...
		dst->lu.p_y = 0;
		v_hide_c(GRPH_Vdi);
		vro_cpyfm(GRPH_Vdi, S_ONLY, (short *) &WIND_SaveArea[0], &scrn, &_WIND_SaveMfdb);
		v_show_c(GRPH_Vdi, 0);
		WIND_SaveArea[2] = *src;
	}
}
...
void WindSaveFlush(BOOL restore)
{
	...
	if (_WIND_SaveMfdb.fd_addr)
	{
		if (restore)
		{
			...
			{
				vro_cpyfm(GRPH_Vdi, S_ONLY, (short *) &WIND_SaveArea[1], &_WIND_SaveMfdb, &scrn);
				WIND_SaveDone = xTrue;
			} else
			{
				...
				while (dst->rd.p_x > 0 && dst->rd.p_y > 0)
				{
					...
					if (GrphIntersectP(dst, &WIND_SaveArea[0]))
					{
						...
						vro_cpyfm(GRPH_Vdi, S_ONLY, &WIND_SaveArea[1].lu.p_x, &_WIND_SaveMfdb, &scrn);
					}

...
#	define SET(R,G,B, C) dst->r = R; dst->g = G; dst->b = B; pixel = C; break
	...
	CARD32 pixel = ((src->r & 0xC000) >> 4) | ((src->g & 0xC000) >> 8) | ((src->b & 0xC000) >> 12);
	...
	default:
		{
			...
			if (pixel <= 0x17FFE)
			{
				if (pixel <= 0x0BFFF)
				{
					dst->r = dst->g = dst->b = 0x3FFF;
					pixel = G_BLACK;
				} else
				{
					dst->r = dst->g = dst->b = 0x7FFF;
					pixel = G_LBLACK;
				}
			} else
			{
...
/* ------------------------------------------------------------------------------ */
static CARD32 _lookup_16(RGB *dst, const RGB *src)
{
...
/* ------------------------------------------------------------------------------ */
static CARD32 _lookup_24_32(RGB *dst, const RGB *src)
{
	CARD32 pixel = ((CARD32) (src->r & 0xF800) << 8) | (src->g & 0xF800) | (src->b >> 8);
...
/* ------------------------------------------------------------------------------ */
static CARD16 _pixel_16(CARD32 pixel)
{
	CARD16 r = pixel & 0xF800;
...
/* ------------------------------------------------------------------------------ */
static CARD16 _pixel_24_32(CARD32 pixel)
{
...
/* ------------------------------------------------------------------------------ */
static const RGB *_Cmap_LookupName(const char *name, size_t len)
{
	typedef struct
	{
		const char *name;
...
CARD16 CmapRevertIdx(CARD16 idx)
{
	...
	short green = ((long) rgb->g * 256) / 1001;
	short blue = ((long) rgb->b * 256) / 1001;
...
/* ------------------------------------------------------------------------------ */
void RQ_CreateColormap(CLIENT *clnt, xCreateColormapReq *q)
{
	PRINT(-X_CreateColormap, " M:%lX W:%lX V:%lX alloc=%s", q->mid, q->window, q->visual, (q->alloc ? "All" : "None"));
}


/* ------------------------------------------------------------------------------ */
void RQ_FreeColormap(CLIENT *clnt, xFreeColormapReq *q)
{
...
/* ------------------------------------------------------------------------------ */
void RQ_CopyColormapAndFree(CLIENT *clnt, xCopyColormapAndFreeReq *q)
{
...
/* ------------------------------------------------------------------------------ */
void RQ_InstallColormap(CLIENT *clnt, xInstallColormapReq *q)
{
...
/* ------------------------------------------------------------------------------ */
void RQ_UninstallColormap(CLIENT *clnt, xUninstallColormapReq *q)
{
...
/* ------------------------------------------------------------------------------ */
void RQ_ListInstalledColormaps(CLIENT *clnt, xListInstalledColormapsReq *q)
{
...
/* ------------------------------------------------------------------------------ */
void RQ_AllocColor(CLIENT *clnt, xAllocColorReq *q)
{
	...
	DEBUG(X_AllocColor, " M:%lX rgb=%04X,%04X,%04X -> %li", q->cmap, q->red, q->green, q->blue, r->pixel);
}


/* ------------------------------------------------------------------------------ */
void RQ_AllocNamedColor(CLIENT *clnt, xAllocNamedColorReq *q)
{
...
/* ------------------------------------------------------------------------------ */
void RQ_AllocColorCells(CLIENT *clnt, xAllocColorCellsReq *q)
{
#if 0
...
/* ------------------------------------------------------------------------------ */
void RQ_AllocColorPlanes(CLIENT *clnt, xAllocColorPlanesReq *q)
{
	PRINT(-X_AllocColorPlanes, " M:%lX %i rgb=%u,%u,%u", q->cmap, q->colors, q->red, q->green, q->blue);
}


/* ------------------------------------------------------------------------------ */
void RQ_FreeColors(CLIENT *clnt, xFreeColorsReq *q)
{
#if 0
...
/* ------------------------------------------------------------------------------ */
void RQ_StoreColors(CLIENT *clnt, xStoreColorsReq *q)
{
	size_t len = ((q->length * 4) - sizeof(xStoreColorsReq)) / sizeof(xColorItem);
...
/* ------------------------------------------------------------------------------ */
void RQ_StoreNamedColor(CLIENT *clnt, xStoreNamedColorReq *q)
{
	PRINT(-X_StoreNamedColor, " M:%lX 0x%lX='%.*s'", q->cmap, q->pixel, q->nbytes, (char *) (q + 1));
}


/* ------------------------------------------------------------------------------ */
void RQ_QueryColors(CLIENT *clnt, xQueryColorsReq *q)
{
	...
	DEBUG(X_QueryColors, "- M:%lX (%lu)", q->cmap, len);
	...
	if (GRPH_Depth <= 8)
	{
		for (i = 0; i < len; i++, pix++, dst++)
		{
			...
			dst->blue = PIXEL(blue);
			DEBUG(0, "+- %lu", pixel);
			DEBUG(0, "+-%04X,%04X,%04X", dst->red, dst->green, dst->blue);
		}
	...
	{									/* (GRPH_Depth == 24/32) */
		...
		{
			CARD32 pixel = (clnt->DoSwap ? Swap32(*pix) : *pix);
			CARD16 red = (pixel >> 16) & 0x00FF;
			CARD16 green = pixel & 0xFF00;
			CARD16 blue = pixel & 0x00FF;
...
/* ------------------------------------------------------------------------------ */
void RQ_LookupColor(CLIENT *clnt, xLookupColorReq *q)
{
	const RGB *rgb = _Cmap_LookupName((char *) (q + 1), q->nbytes);

	if (!rgb)
	{
		Bad(BadName, None, X_LookupColor, "_");
	} else
	{
	...
	DEBUG(X_LookupColor, " M:%lX '%.*s'", q->cmap, q->nbytes, (char *) (q + 1));
}

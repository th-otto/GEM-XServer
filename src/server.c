...
static struct s_setup _SRVR_ConnMSB = {
...
static struct s_setup _SRVR_ConnLSB;

static size_t _SRVR_SetupBytes = sizeof(xConnSetupPrefix) + sizeof(xConnSetup)
	+ Align(sizeof(_SRVR_Vendor) - 1) + sizeof(xWindowRoot);
...
/* ============================================================================== */
void SrvrGrab(p_CLIENT clnt)
{
	SRVR_GrabFD = clnt->FdSet | SRVR_Conn.FdSet;
}

/* ============================================================================== */
void SrvrUngrab(p_CLIENT clnt)
{
...
static BOOL _Srvr_Accept(p_CONNECTION conn, BOOL rd, BOOL wr)
{
	struct hostent *peer;
	const char *addr;
	...
	ClntCreate(fd, (peer ? peer->h_name : "<unknown>"), addr, sock_in.sin_port);
...
/* ============================================================================== */
void SrvrConnInsert(p_CONNECTION conn)
{
...
/* ============================================================================== */
void SrvrConnRemove(p_CONNECTION conn)
{
	if (!conn.p->FdSet || !SRVR_ConnTable[conn.p->Fd].p)
	{
...
size_t SrvrSetup(void *buf, CARD16 maxreqlen, int DoSwap, long rid)
{
	...
	{
		memcpy(p, d[i], sizeof(struct xDepthAndVisual));
		p += sizeof(struct xDepthAndVisual);
		len += sizeof(struct xDepthAndVisual);
	}
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_GrabServer(CLIENT *clnt, xGrabServerReq *q)
{
	/*
...
/*------------------------------------------------------------------------------ */
void RQ_UngrabServer(CLIENT *clnt, xUngrabServerReq *q)
{
	SrvrUngrab(clnt);

	DEBUG(X_UngrabServer, " ");
}


/*------------------------------------------------------------------------------ */
void RQ_Bell(CLIENT *clnt, xBellReq *q)
{
...
/*------------------------------------------------------------------------------ */
void RQ_QueryExtension(CLIENT *clnt, xQueryExtensionReq *q)
{
...
/*------------------------------------------------------------------------------ */
void RQ_ListExtensions(CLIENT *clnt, xListExtensionsReq *_unused_)
{
...
/*------------------------------------------------------------------------------ */
void RQ_SetScreenSaver(CLIENT *clnt, xSetScreenSaverReq *q)
{
	PRINT(-X_SetScreenSaver, " ");
}

/*------------------------------------------------------------------------------ */
void RQ_ForceScreenSaver(CLIENT *clnt, xForceScreenSaverReq *q)
{
	PRINT(-X_ForceScreenSaver, " ");
}

/*------------------------------------------------------------------------------ */
void RQ_GetScreenSaver(CLIENT *clnt, xGetScreenSaverReq *q)
{
...
/*------------------------------------------------------------------------------ */
void RQ_ChangeHosts(CLIENT *clnt, xChangeHostsReq *q)
{
	PRINT(-X_ChangeHosts, " ");
}

/*------------------------------------------------------------------------------ */
void RQ_ListHosts(CLIENT *clnt, xListHostsReq *q)
{
	PRINT(-X_ListHosts, " ");
}

/*------------------------------------------------------------------------------ */
void RQ_SetAccessControl(CLIENT *clnt, xSetAccessControlReq *q)
{
...
/*------------------------------------------------------------------------------ */
void RQ_NoOperation(CLIENT *clnt, xNoOperationReq *_unused_)
{
	DEBUG(X_NoOperation, " ");
}

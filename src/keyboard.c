...
static CARD8 KYBD_Static[][2] = {		/* assignment of keycodes to the bits */
	...
	{ 0, 0 }							/* <-- this one is used to hold a pending keycode */
...
/* ------------------------------------------------------------------------------ */
static void set_sym(KeySym line[], int col, KeySym sym)
{
	line += col;
	*line = sym;
	if (!(col & 1))
...
/* ------------------------------------------------------------------------------ */
static void set_map(const SCANTAB *tab, size_t num)
{
	while (num--)
	{
		...
		if (*k && *k != XK_VoidSymbol)
		{
			x_printf("    * can't map code %i:%i to %04lX '%.1s', is %04lX \n",
			         tab->scan, tab->col, tab->sym, (tab->chr >= ' ' ? (char *) &tab->chr : ""), *k);
		} else
		{
...
/* ------------------------------------------------------------------------------ */
static int ins_map(const char *set)
{
	int scan = 0;
	int num = 0;
	CARD8 asc;
	...
	while ((asc = *(set++)))
	{
		...
		do
		{
			if (++scan > KYBD_CodeMax - KEYSYM_OFFS)
			{
...
/* ------------------------------------------------------------------------------ */
static int analyse_ktbl(const CARD8 *tbl, int shift)
{
	/*
...
/* ------------------------------------------------------------------------------ */
static int analyse_xtbl(const KEYPAIR *tbl, int shift)
{
	/*
	...
	while ((scan = tbl->scan))
	{
		...
		} else
		{
			...
			x_printf("    * duplicate scancode for 0x%02X '%.1s' (0x%02X):"
				   " 0x%02X alt/%s \n",
				   asc, (asc >= ' ' ? (char *) &asc : ""), KYBD_Set[asc], scan, (shift == 0 ? "unshift" : "shift"));
		}
		tbl++;
		n++;
	}
...
void KybdInit(void)
{
	if (!KYBD_Table)
	{
		...
		{
			KYBD_Symbol[i][0] = KYBD_Symbol[i][1] = KYBD_Symbol[i][2] = KYBD_Symbol[i][3] = XK_VoidSymbol;
		}

		/* analyze the standard keytables */
		...
		{								/* analyse extended keytables */
			if ((long) KYBD_Table->AltGr >= os_beg && (long) KYBD_Table->AltGr < os_end)
			{
				KYBD_Atari = xFalse;
				type = "Milan";
		...
		if (KYBD_Atari)
		{
			static SCANTAB const tab[] = {
				{ 113, 2, 0, XK_End },
				{ 99, 2, 0, XK_Page_Up },
				{ 100, 2, 0, XK_Page_Down }
			};
			set_map(tab, numberof(tab));
		...
		}
		{
			...
			static SCANTAB const mod_tab[] = {
				{ KC_SHFT_L, 0, 0, XK_Shift_L },
				{ KC_SHFT_R, 0, 0, XK_Shift_R },
				{ KC_LOCK, 0, 0, XK_Caps_Lock },
				{ KC_CTRL, 0, 0, XK_Control_L },
				{ KC_ALT, 0, 0, XK_Alt_L },
				{ KC_ALTGR, 0, 0, XK_Mode_switch }
			};
			...
			do
			{
				if (KYBD_Symbol[i][0] != XK_VoidSymbol)
				{
		...
		{
			SCANTAB const tab = {
				KYBD_Set['3'], 2, 'þ', XK_threesuperior
			};
			set_map(&tab, 1);
		}
		if (!KYBD_Set['æ'])
		...
		if (KYBD_Atari)
			KYBD_ModMap[3][1] = KEYCODE(KC_ALTGR);
		else
			KYBD_ModMap[4][0] = KEYCODE(KC_ALTGR);

		x_printf("    got keycodes: %i unShift, %i Shift", n_usf, n_sft);
		if (n_alt || n_asf)
			x_printf(", %i Alt, %i AltShft", n_alt, n_asf);
		if (n_agr)
			x_printf(", %i AltGr", n_agr);
		for (i = 1; i < sizeof(KYBD_Set); n_gap += (KYBD_Set[i++] ? 0 : 1))
			;
		x_printf(", %i gap%s\n", n_gap, (n_gap == 1 ? "" : "s"));
		if (n_xtr)
		{
			x_printf("    mapped %i extra keycode%s, ", n_xtr, (n_xtr == 1 ? "" : "s"));
		} else
		{
			x_printf("    ");
		}
...
short KybdEvent(CARD16 scan, CARD8 meta)
{
	...
	if (_WIND_PointerRoot)
	{
		...
		if (*KYBD_Pending)
		{
			EvntPropagate(_WIND_PointerRoot, KeyReleaseMask, KeyRelease, c_id, r_xy, e_xy, *KYBD_Pending);
			*KYBD_Pending = 0;
		}
		while (chng != 0)
		{
			if (chng & 1)
			{
				if (meta & (1 << i))
				{
					...
					EvntPropagate(_WIND_PointerRoot, KeyPressMask, KeyPress, c_id, r_xy, e_xy, KYBD_Static[i][0]);
				} else
				{
					MAIN_Key_Mask &= ~KYBD_Static[i][1] | shft;
					EvntPropagate(_WIND_PointerRoot, KeyReleaseMask, KeyRelease, c_id, r_xy, e_xy, KYBD_Static[i][0]);
				}
			}
			chng >>= 1;
			i++;
		...
		if (code)
		{
			...
			if (scan < 0x0100)
			{
				MAIN_Key_Mask = (Tos2Iso[scan] == KYBD_Symbol[code][0] ? 0 : ShiftMask);
			}
			code = KEYCODE(code);
			EvntPropagate(_WIND_PointerRoot, KeyPressMask, KeyPress, c_id, r_xy, e_xy, code);
			if (scan < 0x0100)
			{
				EvntPropagate(_WIND_PointerRoot, KeyReleaseMask, KeyRelease, c_id, r_xy, e_xy, code);
				MAIN_Key_Mask = save;
			} else
			{
				*KYBD_Pending = code;
...
/* ============================================================================== */
static void Kybd_GetMapping(CARD8 *map)
{
	int i;
	int n = numberof(KYBD_Static) - 1;
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_GetKeyboardControl(CLIENT *clnt, xGetKeyboardControlReq *q)
{
	/*
	...
	unsigned char conterm = Ssystem(S_GETBVAL, 0x484, 0);

	DEBUG(X_GetKeyboardControl, " ");

	r->ledMask = (MAIN_Key_Mask & LockMask ? 1 : 0);
	r->bellPercent = conterm & 0x04 ? 100 : 0;
	r->bellPitch = 2000;				/* Hz */
	r->bellDuration = 100;				/* ms */
	r->keyClickPercent = conterm & 0x01 ? 100 : 0;
	r->globalAutoRepeat = conterm & 0x02 ? xTrue : xFalse;

	if (r->globalAutoRepeat)
		memcpy(r->map, KYBD_Rep, sizeof(r->map));
	else
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_GetModifierMapping(CLIENT *clnt, xGetModifierMappingReq *q)
{
	/*
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_GetKeyboardMapping(CLIENT *clnt, xGetKeyboardMappingReq *q)
{
	/*
	...
	} else
	{
		...
		DEBUG(X_GetKeyboardMapping, " %i (%i)", q->firstKeyCode, q->count);
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_ChangeKeyboardMapping(CLIENT *clnt, xChangeKeyboardMappingReq *q)
{
	/*
	...
	{
		Bad(BadAlloc, None, X_ChangeKeyboardMapping, "_ %i (%i*%i)", q->firstKeyCode, q->keyCodes, q->keySymsPerKeyCode);
	...
	} else
	{
		...
		DEBUG(X_ChangeKeyboardMapping, " %i (%i*%i)", q->firstKeyCode, q->keyCodes, q->keySymsPerKeyCode);
...
/* ------------------------------------------------------------------------------ */
void RQ_ChangeKeyboardControl(CLIENT *clnt, xChangeKeyboardControlReq *q)
{
	PRINT(-X_ChangeKeyboardControl, " ");
}

/* ------------------------------------------------------------------------------ */
void RQ_SetModifierMapping(CLIENT *clnt, xSetModifierMappingReq *q)
{
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_QueryKeymap(CLIENT *clnt, xQueryKeymapReq *q)
{
	/*

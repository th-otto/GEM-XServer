...
/* ============================================================================== */
void *PropValue(const PROPERTIES *pool, Atom name, Atom type, size_t min_len)
{
	void *data = NULL;
	PROPERTY *prop;

	if ((prop = _Prop_Find(pool, name))
		&& (type == None || type == prop->Type) && (!min_len || min_len <= prop->Length))
	{
		data = prop->Data;
	}
	return data;
...
/* ============================================================================== */
BOOL PropHasAtom(const PROPERTIES *pool, Atom name, Atom which)
{
	BOOL has = xFalse;
	PROPERTY *prop;
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_ChangeProperty(CLIENT *clnt, xChangePropertyReq *q)
{
	/*
	...
	} else if (!AtomValid(q->type))
	{
		Bad(BadAtom, q->type, X_ChangeProperty, "_(W:%lX,'%s'):\n"
			"          undefined type.", q->window, ATOM_Table[q->property]->Name);
	} else if (!factor)
	{
		Bad(BadValue, q->format, X_ChangeProperty, "_(W:%lX,'%s'):\n"
			"          illegal format.", q->window, ATOM_Table[q->property]->Name);
	} else if (!wind->Properties && !(pool = malloc(sizeof(PROPERTIES))))
	{
		Bad(BadAlloc, None, X_ChangeProperty, "_(W:%lX,A:%lu): pool struct.", q->window, q->property);
	} else if ((have = _Prop_Find(wind->Properties, q->property)) &&
			   q->mode != PropModeReplace && (q->type != have->Type || q->format != have->Format))
	{
		Bad(BadMatch, None, X_ChangeProperty, "_(W:%lX,'%s'):\n"
			"          type = A:%lu/A:%lu, format = %i/%i.",
			q->window, ATOM_Table[q->property]->Name, q->type, have->Type, q->format, have->Format);
	} else
	{
...
#	ifdef TRACE
		PRINT(X_ChangeProperty, "-(W:%lX): '%s'(%s,%i*%li,%i)",
			  q->window, ATOM_Table[q->property]->Name, ATOM_Table[q->type]->Name, factor, q->nUnits, q->mode);
		if (q->type == XA_STRING)
		{
...
#	endif
		...
		if (!prop)
		{
			Bad(BadAlloc, None, X_ChangeProperty, "_(W:%lX,A:%lX):\n"
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_DeleteProperty(CLIENT *clnt, xDeletePropertyReq *q)
{
	/*
	...
	} else
	{
		...
		DEBUG(X_DeleteProperty, " '%s'(%s) from W:%lX",
			  ATOM_Table[q->property]->Name, (prop ? ATOM_Table[prop->Type]->Name : "n/a"), q->window);
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_GetProperty(CLIENT *clnt, xGetPropertyReq *q)
{
	/*
	...
	if (!wind)
	{
		Bad(BadWindow, q->window, X_GetProperty, "_():\n          not %s.", (DBG_XRSC_TypeError ? "a window" : "found"));
	} else if (!AtomValid(q->property))
	{
		Bad(BadAtom, q->property, X_GetProperty, "_(W:%lX)", q->window);
	} else if ((prop = _Prop_Find(wind->Properties, q->property)) && offs > prop->Length)
	{
		Bad(BadValue, q->longOffset, X_GetProperty, "_(W:%lX,A:%lu)\n"
			"          offset %lu > property length %u.", q->window, q->property, offs, prop->Length);
	} else
	{
		...
		DEBUG(X_GetProperty, " '%s'(%s,%u) %lu-%lu from W:%lX",
			  ATOM_Table[q->property]->Name, ATOM_Table[r->propertyType]->Name, r->format, offs, len, q->window);
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_ListProperties(CLIENT *clnt, xListPropertiesReq *q)
{
	/*
	...
	} else
	{
		...
		CARD16 num = 0;
		size_t size = 0;
		size_t bspc = clnt->oBuf.Size - (clnt->oBuf.Done + clnt->oBuf.Left) - sz_xListPropertiesReply;
		...
		if (pool)
		{
			...
			for (i = 0; i < XrscPOOLSIZE(pool->Pool); ++i)
			{
				...
				while (prop)
				{
					...
					if (need > bspc)
					{
						r = ClntOutBuffer(&clnt->oBuf, sz_xListPropertiesReply + need, sz_xListPropertiesReply + size, xTrue);
						atom = (Atom *) (r + 1) + num;
						bspc = clnt->oBuf.Size - (clnt->oBuf.Done + clnt->oBuf.Left) - sz_xListPropertiesReply;
					}
		...
		DEBUG(X_ListProperties, " of W:%lX (%u)", q->id, num);
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_RotateProperties(CLIENT *clnt, xRotatePropertiesReq *q)
{
	/*
	...
	} else if (q->nAtoms)
	{
		...
		for (i = 0; i < num; i++)
		{
			...
			{
				Bad(BadMatch, None, X_RotateProperties, "_(W:%lX):\n"
					"          property A:%lX not found.", q->window, name[i]);
				ok = xFalse;
				break;
				...
				for (j = 0; j < i; ++j)
				{
					...
					{
						Bad(BadMatch, None, X_RotateProperties, "_(W:%lX):\n"
							"          property A:%lX occured more than once.", q->window, name[i]);
						ok = xFalse;
						i = q->nAtoms;
						break;
					}
		...
		if (ok && delta)
		{
			...
			for (i = 0; i < num; i++)
			{
				...
				if (notify)
					EvntPropertyNotify(wind, name[i], PropertyNewValue);
				if (iccc)
					_Prop_ICCC(wind, prop[i], xTrue);
			}

			DEBUG(X_RotateProperties, "(W:%lX,%u,%+i)", q->window, q->nAtoms, q->nPositions);
		}
	}
}

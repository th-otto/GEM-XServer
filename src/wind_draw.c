...
/* ------------------------------------------------------------------------------ */
static CARD16 clip_children(WINDOW *wind, PXY orig, PRECT *dst, PRECT *clip)
{
	CARD16 num = 0;
	CARD16 cnt = 0;
	CARD16 i;
	...
	if (num)
	{
		...
		do
		{
			...
			for (end = beg + 1; end < num; ++end)
			{
				...
				{
					if (list[end]->lu.p_y <= bot)
						bot = list[end]->lu.p_y - 1;
					break;
			...
			if (i > beg)
			{
				while (i < num && list[i]->lu.p_y == bot && list[i]->lu.p_x < list[i - 1]->lu.p_x)
				{
					short j = i;
					PRECT *save = list[i];
...
/* ============================================================================== */
CARD16 WindClipLock(WINDOW *wind, CARD16 border, const GRECT *clip, short n_clip, PXY *orig, PRECT **pBuf, BOOL incl_chlds)
{
	WINDOW *pwnd;
	WINDOW *twnd = wind;
	...
	while ((pwnd = twnd->Parent) != NULL)
	{
		if (work.g_x + work.g_w > pwnd->Rect.g_w)
			work.g_w = pwnd->Rect.g_w - work.g_x;
		if (work.g_y + work.g_h > pwnd->Rect.g_h)
			work.g_h = pwnd->Rect.g_h - work.g_y;
		if ((work.g_x += pwnd->Rect.g_x) < 0)
		{
			work.g_w += work.g_x;
			work.g_x = 0;
		}
		if ((work.g_y += pwnd->Rect.g_y) < 0)
		{
			work.g_h += work.g_y;
			work.g_y = 0;
		}
		...
		if (pwnd == &WIND_Root || !(visb &= pwnd->isMapped))
			break;
		twnd = pwnd;
	}
	if (!visb || work.g_w <= 0 || work.g_h <= 0)
		return 0;
	...
	*pBuf = sect = (PRECT *) (((unsigned long) (unsigned short) a << 16) | ((unsigned long) (unsigned short) b));
	...
	while (rect.g_w > 0 && rect.g_h > 0)
	{
		...
		do
		{
			sect->lu.p_x = rect.g_x;
			sect->lu.p_y = rect.g_y;
			sect->rd.p_x = rect.g_w;
			sect->rd.p_y = rect.g_h;
...
/* ============================================================================== */
void WindDrawPmap(PIXMAP *pmap, PXY orig, PRECT *sect)
{
	MFDB scrn = { NULL, };
...
#define d_rd pxy[3]
	...
	while ((d = d_rd.p_y - d_lu.p_y) >= 0)
	{
		...
		while ((d = d_rd.p_x - d_lu.p_x) >= 0)
		{
			...
			if (pmap->Depth == 1)
			{
				vrt_cpyfm(GRPH_Vdi, MD_REPLACE, (short *) pxy, PmapMFDB(pmap), &scrn, color);
			} else
			{
...
/* ------------------------------------------------------------------------------ */
int WindDrawBgnd(WINDOW *wind, PXY orig, PRECT *area, PRECT *sect, CARD16 nSct, GRECT *exps)
{
	int cnt = 0;
...
/* ------------------------------------------------------------------------------ */
static void draw_brdr(WINDOW *wind, PRECT *work, PRECT *area, PRECT *sect, int num)
{
...
/* ------------------------------------------------------------------------------ */
static BOOL draw_wind(WINDOW *wind, PRECT *work, PRECT *area, PRECT *sect, int nClp)
{
	/*
	 ...
	 * nClp: list length
	 */
	GRECT *exps = (GRECT *) (wind->u.List.AllMasks & ExposureMask ? area + 2 : NULL);
	PXY orig = work->lu;
	int nEvn = 0;
...
/* ============================================================================== */
void WindDrawSection(WINDOW *wind, const GRECT *clip)
{
	BOOL enter = xTrue;
	int level = 0;
	PXY base;
	...
	if (wind->GwmDecor)
	{
		nClp = WindClipLock(wind, WMGR_Decor, clip, -1, &base, &sect, IncludeInferiors);
		if (nClp)
		{
			PRECT work = { base, {
			...
			if (draw_wind(wind, &work, area, sect, nClp) && (wind = wind->StackBot))
			{
				*area = work;
				level = 1;
			} else
	...
	{
		nClp = WindClipLock(wind, wind->BorderWidth, clip, -1, &base, &sect, IncludeInferiors);
		level = 0;
		area = (sect + nClp);
...
/* ============================================================================== */
void WindPutMono(p_WINDOW wind, p_GC gc, GRECT *rct, MFDB *src)
{
	PRECT *sect;
	PXY orig;
	GRECT *clip;
	...
	if (nClp)
	{
		...
		short mode = (gc->Function == GXor ? MD_TRANS : gc->Function == GXor ? MD_XOR : MD_REPLACE);
		PRECT pxy[2];
...
/* ============================================================================== */
void WindPutColor(p_WINDOW wind, p_GC gc, GRECT *rct, MFDB *src)
{
	PRECT *sect;
	PXY orig;
	GRECT *clip;
...
#if 0
static void
_put_mono(p_WINDOW wind, p_GC gc, PXY offs, MFDB *src, PRECT *sect, CARD16 nSct, PRECT *clip, CARD16 nClp)
{
...
/* ------------------------------------------------------------------------------ */
static void _put_color(p_WINDOW wind, p_GC gc, PXY offs, MFDB *src, const PRECT *sect, CARD16 nSct, const PRECT *clip, CARD16 nClp)
{
...
/* ============================================================================== */
void WindPutImg(p_WINDOW wind, p_GC gc, GRECT *rct, MFDB *src, PXY orig, PRECT *sect, CARD16 nSct)
{
	PRECT *clip;
	CARD16 nClp;
...
/* ============================================================================== */
CARD16 WindScroll(p_WINDOW wind, p_GC gc, GRECT *rect, PXY diff, PXY orig, PRECT *sect, CARD16 nSct, GRECT *exps)
{
	/*
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_ClearArea(CLIENT *clnt, xClearAreaReq *q)
{
	/*
	...
	{
		Bad(BadMatch, None, X_ClearArea, "_(W:%X) class InputOnly", wind->Id);
	} else if ((evn = (q->exposures && (wind->u.List.AllMasks & ExposureMask))) || wind->hasBackGnd)
	{
		PRECT *sect;
		PXY orig;
		CARD16 nClp;
		...
		DEBUG(X_ClearArea, " W:%X [%i,%i/%i,%i]", wind->Id, q->x, q->y, q->width, q->height);
		...
		if (q->width > 0 && q->height > 0
			&& (nClp = WindClipLock(wind, 0, (GRECT *) & q->x, 1, &orig, &sect, IncludeInferiors)))
		{
			...
			if (wind->Id == ROOT_WINDOW)
			{
				...
				if (wind->hasBackGnd)
				{
					do
					{
						...
						objc_draw(&WMGR_Desktop, 0, 1, sect->lu.p_x, sect->lu.p_y, sect->rd.p_x, sect->rd.p_y);

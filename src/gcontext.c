...
/* ============================================================================== */
void GcntDelete(p_GC gc, p_CLIENT clnt)
{
	if (gc->Tile)
...
/* ------------------------------------------------------------------------------ */
static void _Gcnt_setup(CLIENT *clnt, GC *gc, CARD32 mask, CARD32 *val, CARD8 req)
{
	int i = 0;
	...
	{
		gc->PlaneMask = *(val++);
		DEBUG(0, "+- pmsk=%lu", gc->PlaneMask);
	}
	if (mask & GCForeground)
	{
		gc->Foreground = CmapPixelIdx(*(val++), gc->Depth);
		DEBUG(0, "+- fgnd=%lu", gc->Foreground);
	}
	if (mask & GCBackground)
	{
		gc->Background = CmapPixelIdx(*(val++), gc->Depth);
		DEBUG(0, "+- bgnd=%lu", gc->Background);
	}
	if (mask & GCLineWidth)
	{
		gc->LineWidth = *(val++);
		DEBUG(0, "+- lwid=%u", gc->LineWidth);
	}
	if (mask & GCLineStyle)
	{
		CARD8 style = *(val++);
	...
	if (mask & GCTile)
	{
		...
		{
			PRINT(0, " ");
			Bad(BadMatch, None, req, "_          Tile depth %u not %u.", tile->Depth, gc->Depth);
			return;
		} else
		{
			DEBUG(0, "+- tile=0x%lX", *val);
			if (gc->Tile)
			{
				PmapFree(gc->Tile, NULL);
			}
	...
	if (mask & GCStipple)
	{
		...
		{
			PRINT(0, " ");
			Bad(BadMatch, None, req, "_          Stipple depth %u.", stip->Depth);
			return;
		} else
		{
			DEBUG(0, "+- stip=0x%lX", *val);
			if (gc->Stipple)
			{
				PmapFree(gc->Stipple, NULL);
			}
	...
	{
		gc->TileStip.p_x = *(val++);
		DEBUG(0, "+- stpx=%i", gc->TileStip.p_x);
	}
	if (mask & GCTileStipYOrigin)
	{
		gc->TileStip.p_y = *(val++);
		DEBUG(0, "+- stpy=%i", gc->TileStip.p_y);
	}
	if (mask & GCFont)
	{
	...
	{
		gc->Clip.p_x = *(val++);
		DEBUG(0, "+- clpx=%i", gc->Clip.p_x);
	}
	if (mask & GCClipYOrigin)
	{
		gc->Clip.p_y = *(val++);
		DEBUG(0, "+- clpy=%i", gc->Clip.p_y);
	}
	if (mask & GCClipMask)
	{
		if (*val == None)
		...
		} else
		{
			...
			{
				PRINT(0, " ");
				Bad(BadMatch, None, req, "_          clip-mask depth %u.", clip->Depth);
				return;
			} else
			{
				DEBUG(0, "+- clip=P:%X", clip->Id);
	...
	{
		gc->DashOffset = *(val++);
		DEBUG(0, "+- doff=%i", gc->DashOffset);
	}
	if (mask & GCDashList)
	{
		gc->DashList = *(val++);
		DEBUG(0, "+- dlst=%i", gc->DashList);
	}
	if (mask & GCArcMode)
	{
		CARD8 mode = *(val++);
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_CreateGC(CLIENT *clnt, xCreateGCReq *q)
{
	/*
	...
	{
		Bad(BadAlloc, None, X_CreateGC, "_");
	} else
	{
		DEBUG(X_CreateGC, "- G:%lX for D:%lX", q->gc, q->drawable);
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_ChangeGC(CLIENT *clnt, xChangeGCReq *q)
{
	/*
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_CopyGC(CLIENT *clnt, xCopyGCReq *q)
{
	/*
	...
	{
		Bad(BadMatch, None, X_CopyGC, "_(G:%lX,G:%lX):\n          source depth %u is not %u.",
			q->srcGC, q->dstGC, src->Depth, dst->Depth);
	} else if (q->mask & ~((2uL << GCLastBit) - 1))
	{
		Bad(BadValue, q->mask, X_CopyGC, "_(G:%lX,G:%lX):\n"
			"          invalid value mask 0x%lX.", q->srcGC, q->dstGC, q->mask);
	} else
	{
		DEBUG(X_CopyGC, " G:%lX to G:%lX mask=%06lX", q->srcGC, q->dstGC, q->mask);

		if (q->mask & GCTile)
		{
			if (dst->Tile)
				PmapFree(dst->Tile, NULL);
			dst->Tile = PmapShare(src->Tile);
		}
		if (q->mask & GCStipple)
		{
			if (dst->Stipple)
				PmapFree(dst->Stipple, NULL);
		...
		if (q->mask & GCFont)
			FontCopy((p_FONTABLE) dst, (p_FONTABLE) src);
		if (q->mask & GCFunction)
			dst->Function = src->Function;
		if (q->mask & GCGraphicsExposures)
			dst->GraphExpos = src->GraphExpos;
		if (q->mask & GCSubwindowMode)
			dst->SubwindMode = src->SubwindMode;
		if (q->mask & GCDashList)
			dst->DashList = src->DashList;
		if (q->mask & GCDashOffset)
			dst->DashOffset = src->DashOffset;
		if (q->mask & GCLineWidth)
			dst->LineWidth = src->LineWidth;
		if (q->mask & GCLineStyle)
			dst->LineStyle = src->LineStyle;
		if (q->mask & GCCapStyle)
			dst->CapStyle = src->CapStyle;
		if (q->mask & GCJoinStyle)
			dst->JoinStyle = src->JoinStyle;
		if (q->mask & GCFillStyle)
			dst->FillStyle = src->FillStyle;
		if (q->mask & GCFillRule)
			dst->FillRule = src->FillRule;
		if (q->mask & GCArcMode)
			dst->ArcMode = src->ArcMode;
		if (q->mask & GCPlaneMask)
			dst->PlaneMask = src->PlaneMask;
		if (q->mask & GCForeground)
			dst->Foreground = src->Foreground;
		if (q->mask & GCBackground)
			dst->Background = src->Background;
		if (q->mask & GCTileStipXOrigin)
			dst->TileStip.p_x = src->TileStip.p_x;
		if (q->mask & GCTileStipYOrigin)
			dst->TileStip.p_y = src->TileStip.p_y;
		if (q->mask & GCClipXOrigin)
			dst->Clip.p_x = src->Clip.p_x;
		if (q->mask & GCClipYOrigin)
			dst->Clip.p_y = src->Clip.p_y;
	}
}

/* ------------------------------------------------------------------------------ */
void RQ_SetDashes(CLIENT *clnt, xSetDashesReq *q)
{
	/*
	 * GContext gc:
	 * CARD16   dashOffset:
	 ...
	 *...........................................................................
	 */
	PRINT(-X_SetDashes, " G:%lX offs=%u (%u)", q->gc, q->dashOffset, q->nDashes);
}

/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_SetClipRectangles(CLIENT *clnt, xSetClipRectanglesReq *q)
{
	/*
	...
	GC *gc = GcntFind(q->gc);
	GRECT *clp = NULL;
	size_t num = ((q->length * 4) - sizeof(xSetClipRectanglesReq)) / sizeof(GRECT);
	...
	{
		Bad(BadAlloc, None, X_SetClipRectangles, "_");
	} else
	{
		DEBUG(X_SetClipRectangles, " G:%lX (%i,%i) %li", q->gc, q->xOrigin, q->yOrigin, num);
...
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
void RQ_FreeGC(CLIENT *clnt, xFreeGCReq *q)
{
	/*

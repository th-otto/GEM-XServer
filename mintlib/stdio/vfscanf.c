...
#ifdef __MINT__
# define __strtoll_internal(s, p, b, f) strtoll (s, p, b)
# define __strtoull_internal(s, p, b, f) strtoull (s, p, b)
# define __strtol_internal(s, p, b, f) strtol (s, p, b)
# define __strtoul_internal(s, p, b, f) strtoul (s, p, b)
# define __strtold_internal(s, p, f) strtold (s, p)
# define __strtod_internal(s, p, f) strtod (s, p)
# define __strtof_internal(s, p, f) strtof (s, p)
# undef vfscanf
#endif
...
#else
int
vfscanf (FILE *s, const char *format, va_list argptr)
#endif
{
  va_list arg;
...
# ifndef ONLY_INTEGER_IO
  ...
  /* We must not react on white spaces immediately because they can
     ...
     available anymore.  */
  int skip_space = 0;
  /* Handle the case that we were fooled with "0[xX]" in the input
     string for "%i" and there is no hex digit after `x'.  */
  int skip_x = 0;
...
# endif
      ...
      if (skip_space || (fc != '[' && fc != 'c' && fc != 'C' && fc != 'n'))
	{
	  ...
	  __set_errno (0);
	  /* There cannot be an 'x' left in skip_x in this situation.  */
	  do
	    if (inchar () == EOF && errno == EINTR)
	      input_error ();
	  while (isspace (c));
	      ...
	      if (width != 0 && _tolower (c) == 'x')
		{
		  ...
		  if (base == 16)
			...
			--width;
		      ...
		      if (!isxdigit (c))
		        {
		          /* We now have to distinguish.  If the format
		             had %i then we have read a valid number, namely
		             a zero.  If we saw an explicit %x or %X we
		             have a conversion error.  In any case we have to
		             ungetc the character that is not part of the
		             number.  */
		          ungetc (c, s);
		          ...
		          if (fc != 'i')
		            conv_error ();
		          /* else */
		          /* Department of dirty tricks.  Actually we
		             also have to ungetc the `x' but that
		             would back up 2 characters.  We set the
		             remaining width to 0 which will prevent
		             further reading and save the `x' read in
		             vain.  */
		          skip_x = saved_c;
		          width = 0;
		        }

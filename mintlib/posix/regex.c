...
#if !defined (__MINT__) && (defined _LIBC || WIDE_CHAR_SUPPORT)
...
#if !defined (__MINT__) && (defined HAVE_LIBINTL_H || defined _LIBC)
# include <libintl.h>
#else
...
#define EXTEND_BUFFER()							\
  do { 									\
    long old_buffer = (long)bufp->buffer;				\
    ...
    if (old_buffer != (long)bufp->buffer)					\
      {									\
        b = ((long)b - old_buffer) + bufp->buffer;				\
        begalt = ((long)begalt - old_buffer) + bufp->buffer;			\
        if (fixup_alt_jump)						\
          fixup_alt_jump = ((long)fixup_alt_jump - old_buffer) + bufp->buffer;\
        if (laststart)							\
          laststart = ((long)laststart - old_buffer) + bufp->buffer;		\
        if (pending_exact)						\
          pending_exact = ((long)pending_exact - old_buffer) + bufp->buffer;	\
      }									\
  } while (0)
...
#if !defined __MINT__ \
  && (defined _LIBC || WIDE_CHAR_SUPPORT)
...
/* This is a separate function so that we can force an alloca cleanup
   afterwards.  */
static int
re_match_2_internal (struct re_pattern_buffer *bufp, const char *string1, int size1, const char *string2, int size2, int pos, struct re_registers *regs, int stop)
{
  /* General temporaries.  */
  int mcnt;
  unsigned char *p1;
...
static int
bcmp_translate (const char *s1, const char *s2, int len, RE_TRANSLATE_TYPE translate)
{
